...
public partial class FiscalViewModel : ObservableObject
{
    ...
    private async Task EmitInvoiceAsync()
    {
        ...
        try
        {
            var request = new NextVent.Core.Models.FacturamaCfdiRequest
            {
            ...
            {
                request.Items.Add(new NextVent.Core.Models.CfdiItem
                {
                    ...
                    Taxes = new System.Collections.Generic.List<NextVent.Core.Models.CfdiTax>
                    {
                        new NextVent.Core.Models.CfdiTax { Name = "IVA", Rate = 0.16m, IsRetention = false, Base = (decimal)item.TotalPrice, Total = (decimal)item.TotalPrice * 0.16m }
                    }
                });
            }
