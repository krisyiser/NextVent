...
public class SecurityInterceptionService : ISecurityInterceptionService
{
    ...
    public async Task<NextVent.Core.Helpers.Result> ValidatePinAsync(string username, string inputPin)
    {
        // 1. RATE LIMITING (Bloqueo en memoria)
        if (_failedAttempts.TryGetValue(username, out var record) && record.LockoutEnd > DateTime.Now)
        {
            return NextVent.Core.Helpers.Result.Failure($"Usuario bloqueado. Intente de nuevo en {(record.LockoutEnd - DateTime.Now).Seconds}s.");
        }

        var user = await _userRepository.GetByUsernameAsync(username);
        if (user == null) return NextVent.Core.Helpers.Result.Failure("Usuario no encontrado.");
        ...
        if (user.PasswordSalt == null || user.PasswordHashBytes == null)
        {
            if (user.PinCode == inputPin)
            {
                ...
                return NextVent.Core.Helpers.Result.Success();
            }
        }
        else
        {
        ...
        return NextVent.Core.Helpers.Result.Failure("PIN Incorrecto.");
    }
